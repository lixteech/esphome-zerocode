...
const BOARD_DB = [
  // ESP32
  { value:"esp32",         label:"ESP32 DevKit v1",          platform:"esp32",   board:"esp32dev",                family:"ESP32",    pins:36 },
  { value:"esp32_wroom32", label:"ESP32-WROOM-32E",          platform:"esp32",   board:"esp32dev",                family:"ESP32",    pins:36 },
  { value:"esp32_wrover",  label:"ESP32-WROVER-B (PSRAM)",   platform:"esp32",   board:"esp32dev",                family:"ESP32",    pins:36 },
  { value:"esp32_pico",    label:"ESP32-PICO-D4",            platform:"esp32",   board:"pico32",                  family:"ESP32",    pins:32 },
  { value:"esp32_cam",     label:"ESP32-CAM (AI-Thinker)",   platform:"esp32",   board:"esp32cam",                family:"ESP32",    pins:16 },
  { value:"esp32_poe",     label:"OLIMEX ESP32-POE",         platform:"esp32",   board:"esp32-poe",               family:"ESP32",    pins:32 },
  { value:"ttgo_display",  label:"TTGO T-Display 1.14\"",    platform:"esp32",   board:"ttgo-t-display-1.14inch", family:"ESP32",    pins:32 },
  { value:"ttgo_t7",       label:"TTGO T7 v1.4",             platform:"esp32",   board:"ttgo-lora32-v1",          family:"ESP32",    pins:32 },
  { value:"m5stack_core",  label:"M5Stack Core",             platform:"esp32",   board:"m5stack-core-esp32",      family:"ESP32",    pins:16 },
  { value:"m5stack_atom",  label:"M5Stack Atom Lite",        platform:"esp32",   board:"m5stack-atom",            family:"ESP32",    pins:6  },
  { value:"m5stack_stamp", label:"M5Stack StampS3",          platform:"esp32",   board:"m5stack-stamps3",         family:"ESP32-S3", pins:20 },
  { value:"cyd",           label:"CYD ESP32-2432S028",       platform:"esp32",   board:"esp32dev",                family:"ESP32",    pins:36 },
  { value:"cyd2usb",       label:"CYD2 ESP32-2432S028 v2",   platform:"esp32",   board:"esp32dev",                family:"ESP32",    pins:36 },
  // ESP32-S2
  { value:"esp32s2",       label:"ESP32-S2 (LiLyGO Mini)",   platform:"esp32",   board:"lolin_s2_mini",           family:"ESP32-S2", pins:36 },
  { value:"esp32s2_devkit",label:"ESP32-S2 DevKit M-1",      platform:"esp32",   board:"esp32-s2-saola-1",        family:"ESP32-S2", pins:36 },
  // ESP32-S3
  { value:"esp32s3",       label:"ESP32-S3 DevKitC-1",       platform:"esp32",   board:"esp32-s3-devkitc-1",      family:"ESP32-S3", pins:36 },
  { value:"esp32s3_xiao",  label:"Seeed XIAO ESP32-S3",      platform:"esp32",   board:"seeed_xiao_esp32s3",      family:"ESP32-S3", pins:11 },
  { value:"esp32s3_lipo",  label:"LilyGO T-Display S3",      platform:"esp32",   board:"lilygo-t-display-s3",     family:"ESP32-S3", pins:20 },
  { value:"esp32s3_zero",  label:"WeAct ESP32-S3 Zero",      platform:"esp32",   board:"esp32-s3-devkitc-1",      family:"ESP32-S3", pins:20 },
  // ESP32-C3
  { value:"esp32c3",       label:"ESP32-C3 (XIAO Seeed)",    platform:"esp32",   board:"seeed_xiao_esp32c3",      family:"ESP32-C3", pins:11 },
  { value:"esp32c3_mini",  label:"ESP32-C3 SuperMini",       platform:"esp32",   board:"esp32-c3-devkitm-1",      family:"ESP32-C3", pins:13 },
  { value:"esp32c3_devkit",label:"ESP32-C3 DevKitM-1",       platform:"esp32",   board:"esp32-c3-devkitm-1",      family:"ESP32-C3", pins:22 },
  // ESP32-C6
  { value:"esp32c6",       label:"ESP32-C6 DevKitC-1",       platform:"esp32",   board:"esp32-c6-devkitc-1",      family:"ESP32-C6", pins:22 },
  { value:"esp32c6_xiao",  label:"Seeed XIAO ESP32-C6",      platform:"esp32",   board:"seeed_xiao_esp32c6",      family:"ESP32-C6", pins:11 },
  // ESP32-H2
  { value:"esp32h2",       label:"ESP32-H2 DevKitM-1",       platform:"esp32",   board:"esp32-h2-devkitm-1",      family:"ESP32-H2", pins:20 },
  // ESP8266
  { value:"esp8266",       label:"ESP8266 NodeMCU v2",        platform:"esp8266", board:"nodemcuv2",               family:"ESP8266",  pins:16 },
  { value:"d1mini",        label:"Wemos D1 Mini",             platform:"esp8266", board:"d1_mini",                 family:"ESP8266",  pins:11 },
  { value:"d1mini_pro",    label:"Wemos D1 Mini Pro",         platform:"esp8266", board:"d1_mini_pro",             family:"ESP8266",  pins:11 },
  { value:"esp01",         label:"ESP-01 / ESP-01S",          platform:"esp8266", board:"esp01",                   family:"ESP8266",  pins:4  },
  { value:"esp8285",       label:"ESP8285 (Sonoff Basic)",     platform:"esp8266", board:"esp8285",                 family:"ESP8285",  pins:8  },
  { value:"sonoff_mini",   label:"Sonoff Mini R2",            platform:"esp8266", board:"esp8285",                 family:"ESP8285",  pins:4  },
];
...
/* ============================================================
   YAML GENERATOR — no API encryption, no OTA password
   ============================================================ */
const generateYAML = ({ boardDef, deviceName, wifiSsid, wifiPass, components, services }) => {
  const name = (deviceName || "my-device").replace(/\s+/g, "-").toLowerCase();
...
`;
  ...
  // Sensors
  const sensors = components.filter(c => ALL_COMPONENTS.find(d => d.type === c.type)?.platform === "sensor");
  if (sensors.length > 0) {
    y += `\n# — Sensors ———————————————————————————————————————————————\nsensor:\n`;
    sensors.forEach(c => {
  ...
  // Binary sensors
  const binaries = components.filter(c => ALL_COMPONENTS.find(d => d.type === c.type)?.platform === "binary");
  ...
  if (fans.length > 0) {
    y += `fan:\n`;
    fans.forEach(c => { y += `  - platform: speed\n    output: fan_output_${c.id}\n    name: "${c.name||"Fan"}"\n\n`; });
    y += `output:\n`;
    fans.forEach(c => { y += `  - platform: ledc\n    pin: GPIO${c.gpio||"13"}\n    id: fan_output_${c.id}\n\n`; });
  }
...
/* ============================================================
   ...
   ============================================================ */
const C = {
  comment: "#6272a4", key: "#79c0ff", colon: "#8b949e", string: "#a5d6ff",
  bool: "#ff7b72", number: "#f9c74f", plain: "#c9d1d9", indent: "#1e3a5a", dash: "#ff7b72",
};
...
function YAMLLine({ line, num }) {
  ...
  if (kvMatch) {
    ...
    if (v) {
      ...
      if (v === "true" || v === "false")
        tokens.push(<span key="v" style={{ color:C.bool }}>{v}</span>);
      else if (v.startsWith('"'))
        tokens.push(<span key="v" style={{ color:C.string }}>{v}</span>);
      else if (/^0x[0-9a-fA-F]+$/.test(v) || /^-?\d/.test(v))
        tokens.push(<span key="v" style={{ color:C.number }}>{v}</span>);
      else
        tokens.push(<span key="v" style={{ color:C.plain }}>{v}</span>);
    }
  } else {
...
/* ============================================================
   2D BOARD PREVIEW — SVG chip diagram
   ============================================================ */
const FAMILY_COLORS = {
  "ESP32":    { pcb:"#0a2218", chip:"#0d2f10", accent:"#00e5a0", glow:"#00ff88" },
  "ESP32-S2": { pcb:"#0a1a2e", chip:"#0d1f3c", accent:"#00b4ff", glow:"#00cfff" },
  "ESP32-S3": { pcb:"#150d2e", chip:"#1a0f3a", accent:"#a855f7", glow:"#c084fc" },
  "ESP32-C3": { pcb:"#1a120a", chip:"#2a1a08", accent:"#fb923c", glow:"#fdba74" },
  "ESP32-C6": { pcb:"#0a1a1a", chip:"#0d2a2a", accent:"#2dd4bf", glow:"#5eead4" },
  "ESP32-H2": { pcb:"#1a0a12", chip:"#2a0d1a", accent:"#f472b6", glow:"#f9a8d4" },
  "ESP8266":  { pcb:"#1a150a", chip:"#2a1f08", accent:"#facc15", glow:"#fde047" },
  "ESP8285":  { pcb:"#1a150a", chip:"#2a1f08", accent:"#f59e0b", glow:"#fcd34d" },
};

// Generic pin layout: left 9 pins, right 9 pins
const LEFT_PINS  = ["3V3","EN","36","39","34","35","32","33","25"];
const RIGHT_PINS = ["VIN","GND","23","22","TX","RX","21","19","18"];

function BoardPreview({ boardDef, activeGpios }) {
  const theme = FAMILY_COLORS[boardDef?.family] || FAMILY_COLORS["ESP32"];
  const activeSet = new Set(activeGpios.map(String));

  const W = 340, H = 400;
  const PCB_W = 170, PCB_H = 300;
  const cx = W / 2, cy = H / 2;
  const PCB_X = cx - PCB_W / 2, PCB_Y = cy - PCB_H / 2;

  const PIN_COUNT = 9;
  const PIN_SPACING = (PCB_H - 40) / (PIN_COUNT - 1);
  const PIN_R = 5;
  const PIN_LEN = 22;
  ...
  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} style={{ display:"block" }}>
      ...
      {/* PCB board */}
      <rect x={PCB_X} y={PCB_Y} width={PCB_W} height={PCB_H} rx={10} ry={10}
        fill={theme.pcb} stroke={theme.accent} strokeWidth="1" strokeOpacity="0.35" />

      {/* PCB grid lines */}
      {[0.25,0.5,0.75].map((t,i) => (
        <line key={i} x1={PCB_X+10} y1={PCB_Y + PCB_H*t} x2={PCB_X+PCB_W-10} y2={PCB_Y+PCB_H*t}
          stroke={theme.accent} strokeWidth="0.3" strokeOpacity="0.12" />
      ))}
      {[0.33,0.66].map((t,i) => (
        <line key={i} x1={PCB_X + PCB_W*t} y1={PCB_Y+10} x2={PCB_X+PCB_W*t} y2={PCB_Y+PCB_H-10}
          stroke={theme.accent} strokeWidth="0.3" strokeOpacity="0.12" />
      ))}

      {/* USB connector */}
      <rect x={cx-18} y={PCB_Y-14} width={36} height={16} rx={4}
        fill="#1a1a1a" stroke="#333" strokeWidth="1" />
      <rect x={cx-12} y={PCB_Y-11} width={24} height={10} rx={2}
        fill="#111" stroke="#222" strokeWidth="0.5" />

      {/* Antenna */}
      <rect x={PCB_X+PCB_W-24} y={cy-30} width={20} height={60} rx={4}
        fill={theme.chip} stroke={theme.accent} strokeWidth="0.5" strokeOpacity="0.4" />
      <text x={PCB_X+PCB_W-14} y={cy+4} textAnchor="middle" fontSize="6"
        fill={theme.accent} opacity="0.5" fontFamily="monospace" transform={`rotate(-90,${PCB_X+PCB_W-14},${cy})`}>ANTENNA</text>

      {/* Main chip */}
      <rect x={cx-38} y={cy-42} width={76} height={84} rx={6}
        fill="url(#chip-grad)" stroke={theme.accent} strokeWidth="1" strokeOpacity="0.5" />
      {/* Chip mark */}
      <circle cx={cx-28} cy={cy-32} r={4} fill={theme.accent} opacity="0.25" />
      {/* Chip label */}
      <text x={cx} y={cy-8} textAnchor="middle" fontSize="9" fontWeight="700"
        fill={theme.accent} opacity="0.9" fontFamily="monospace">{boardDef?.family}</text>
      <text x={cx} y={cy+6} textAnchor="middle" fontSize="7"
        fill={theme.accent} opacity="0.55" fontFamily="monospace">{boardDef?.board?.slice(0,12)}</text>
      {/* Chip glow pulse */}
      <rect x={cx-38} y={cy-42} width={76} height={84} rx={6}
        fill={theme.glow} opacity={0.04 + pulse * 0.04} />

      {/* LED status dot */}
      <circle cx={PCB_X+16} cy={PCB_Y+16} r={4}
      ...
      {LEFT_PINS.map((label, i) => {
        ...
        const active = isNum && activeSet.has(label);
        return (
          <g key={`l${i}`}>
            <line x1={PCB_X} y1={py} x2={PCB_X - PIN_LEN} y2={py}
              stroke={active ? theme.glow : "#1a2e1a"} strokeWidth={active ? 2 : 1}
              filter={active ? "url(#glow-soft)" : "none"} />
            <circle cx={PCB_X - PIN_LEN} cy={py} r={PIN_R}
            ...
            <text x={PCB_X - PIN_LEN - 8} y={py + 3.5} textAnchor="end" fontSize="7.5"
              fill={active ? theme.accent : "#2d4030"} fontFamily="monospace"
      ...
      {RIGHT_PINS.map((label, i) => {
        ...
        return (
          <g key={`r${i}`}>
            <line x1={PCB_X + PCB_W} y1={py} x2={PCB_X + PCB_W + PIN_LEN} y2={py}
              stroke={active ? theme.glow : "#1a2e1a"} strokeWidth={active ? 2 : 1}
              filter={active ? "url(#glow-soft)" : "none"} />
            <circle cx={PCB_X + PCB_W + PIN_LEN} cy={py} r={PIN_R}
              fill={active ? theme.glow : "#0d1a10"}
              stroke={active ? theme.glow : "#1e3020"}
              strokeWidth="1"
              filter={active ? "url(#glow-strong)" : "none"}
              opacity={active ? 0.7 + pulse*0.3 : 1} />
            <text x={PCB_X + PCB_W + PIN_LEN + 8} y={py + 3.5} textAnchor="start" fontSize="7.5"
              fill={active ? theme.accent : "#2d4030"} fontFamily="monospace"
              fontWeight={active ? "700" : "400"}>{label}</text>
          </g>
        );
      ...
      {/* Family badge */}
      <rect x={PCB_X+6} y={PCB_Y+PCB_H-22} width={PCB_W-12} height={16} rx={4}
        fill={theme.accent} opacity="0.08" />
      <text x={cx} y={PCB_Y+PCB_H-12} textAnchor="middle" fontSize="8"
        fill={theme.accent} opacity="0.5" fontFamily="monospace">{boardDef?.label}</text>
    </svg>
  );
}
...
const inp = {
  ...
  borderRadius:8, padding:"10px 14px", color:"#e2e8f0", fontSize:14, width:"100%",
...
function Field({ label, value, onChange, type="text", placeholder }) {
  return (
    <label style={{ display:"flex", flexDirection:"column", gap:6 }}>
      {label && <span style={{ fontSize:11, color:"#475569", letterSpacing:".07em", textTransform:"uppercase" }}>{label}</span>}
  ...
  return (
    <label style={{ display:"flex", flexDirection:"column", gap:6 }}>
      ...
      <select value={value} onChange={e=>onChange(e.target.value)}
        ...
        {options.map(o=><option key={o.v||o.value} value={o.v||o.value} style={{background:"#0a1220"}}>{o.l||o.label}</option>)}
      </select>
    </label>
  );
}
...
function Toggle({ label, desc, value, onChange }) {
  return (
    <motion.button onClick={()=>onChange(!value)}
      ...
      <div style={{ width:36, height:20, borderRadius:10, padding:2, background: value?"#6366f1":"#1e293b", transition:"background .2s", flexShrink:0, display:"flex", alignItems:"center" }}>
...
const BOARD_FAMILIES = [...new Set(BOARD_DB.map(b => b.family))];
...
function CompPicker({ onAdd }) {
  ...
  const items = q
    ? ALL_COMPONENTS.filter(c => c.label.toLowerCase().includes(q.toLowerCase()) || c.desc.toLowerCase().includes(q.toLowerCase()) || c.tags?.some(t=>t.includes(q.toLowerCase())))
    : COMPONENT_DB[cat] || [];
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:4, padding:"8px 10px", borderBottom:"1px solid rgba(255,255,255,.04)" }}>
                {CATEGORIES.map(c => (
                  <button key={c.id} onClick={()=>setCat(c.id)} style={{
                    ...
                    color: cat===c.id?"#a5b4fc":"#475569", fontSize:11, fontFamily:"inherit", transition:"all .15s",
                  }}>{c.label}</button>
                ))}
              </div>
            )}
...
function CompCard({ comp, onRemove, onChange }) {
  ...
  return (
    <motion.div layout initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:.96}}
      style={{ background:"rgba(255,255,255,.025)", border:"1px solid rgba(255,255,255,.06)", borderRadius:10, overflow:"hidden", marginBottom:6 }}>
      <div onClick={()=>setOpen(!open)} style={{ display:"flex", alignItems:"center", padding:"10px 12px", cursor:"pointer", gap:10 }}>
...
export default function App() {
  ...
  const boardDef = useMemo(() => BOARD_DB.find(b => b.value === boardValue) || BOARD_DB[0], [boardValue]);
  const config   = useMemo(()=>({ boardDef, deviceName, wifiSsid, wifiPass, components, services }), [boardDef,deviceName,wifiSsid,wifiPass,components,services]);
  ...
  // Group boards for select
  const boardOptions = BOARD_DB.map(b => ({ v: b.value, l: `[${b.family}] ${b.label}` }));
    ...
    <div style={{ minHeight:"100vh", background:"#080e1a", color:"#e2e8f0", display:"flex", flexDirection:"column", fontFamily:"'JetBrains Mono','Fira Code',monospace", overflow:"hidden" }}>
      ...
      <header style={{
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:11,color:"#334155",fontFamily:"monospace"}}>{boardDef.family} · {boardDef.pins} pins</span>
      ...
      <div style={{ flex:1, display:"grid", gridTemplateColumns:"340px 1fr 420px", height:"calc(100vh - 52px)", overflow:"hidden" }}>
        ...
        <div style={{ borderRight:"1px solid rgba(255,255,255,.05)", display:"flex", flexDirection:"column", overflow:"hidden", background:"rgba(6,11,20,.7)" }}>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"16px 20px", display:"flex", flexDirection:"column", gap:20 }}>
            ...
            <section>
              ...
              <div style={{ display:"grid", gap:10 }}>
                <Field value={wifiSsid} onChange={setWifiSsid} placeholder="your_wifi_ssid" />
                <Field value={wifiPass} onChange={setWifiPass} placeholder="••••••••" type="password" />
              </div>
            </section>
        ...
        <div style={{ position:"relative", background:"#060c18", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
          ...
          <div style={{ position:"absolute", bottom:14, left:"50%", transform:"translateX(-50%)", fontSize:10, color:"#1e3a5a", fontFamily:"monospace", whiteSpace:"nowrap" }}>
        ...
        <div style={{ borderLeft:"1px solid rgba(255,255,255,.05)", display:"flex", flexDirection:"column", background:"rgba(5,9,18,.8)", overflow:"hidden" }}>
          ...
          <div style={{ padding:"8px 14px", borderBottom:"1px solid rgba(255,255,255,.04)", display:"flex", alignItems:"center", gap:6, background:"rgba(0,0,0,.2)", flexShrink:0 }}>
            ...
            <span style={{ marginLeft:8, fontSize:11, color:"#334155", fontFamily:"monospace" }}>{(deviceName||"my-device").replace(/\s+/g,"-").toLowerCase()}.yaml</span>
          </div>

          <div style={{ flex:1, overflowY:"auto", fontSize:12.5, lineHeight:"20px", padding:"12px 0 20px 0", background:"rgba(4,8,16,.6)" }}>
            {lines.map((line,i) => <YAMLLine key={i} line={line} num={i+1} />)}
          </div>
